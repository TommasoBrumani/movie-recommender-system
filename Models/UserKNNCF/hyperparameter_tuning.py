...
def hyperparameter_tuning():
    ...
    runHyperparameterSearch_Collaborative(recommender_class,
                                    ...
                                    save_model="best",
                                    similarity_type_list=["cosine", "adjusted", "asymmetric", "jaccard", "tanimoto", "dice", "tversky"],
                                    n_cases=n_cases,
                                    n_random_starts=n_random_starts)

    ################### VISUALIZE RESULTS FOR HYPERPARAMETER SEARCH ########################

    # for similarity in ["cosine", "adjusted", "asymmetric", "jaccard", "tanimoto", "dice", "tversky"]:
    #
    #     data_loader = DataIO(folder_path=output_folder_path)
    #     search_metadata = data_loader.load_data(recommender_class.RECOMMENDER_NAME + "_ICM_mixed_" + similarity + "_metadata.zip")
    #
    ...
    for norm in [True, False]:
        for weighting in ["BM25", "TF-IDF", "none"]:
            ...
            search_metadata = data_loader.load_data(
                recommender_class.RECOMMENDER_NAME + "_{}_".format(norm) + weighting + "_metadata.zip")
